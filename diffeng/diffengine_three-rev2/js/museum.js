// Uses classic (non-module) Three.js scripts loaded by museum.html.
...
function start(identity){
  ...
  if (!THREE || !OrbitControls || !GLTFLoader) {
    ...
    message.textContent = 'Three.js failed to load. If you are on a restricted network, try a different connection or self-host the three.js files.';
    return;
  }

  if (location.protocol === 'file:') {
    message.textContent = 'Tip: serve this folder via a local web server (python3 -m http.server) so textures/models load reliably.';
  }
